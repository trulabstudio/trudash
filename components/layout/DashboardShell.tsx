...
>
  ...
  {siteConfig.name}
            ...
            <div className="ml-auto flex items-center gap-3 text-sm text-muted-foreground">
              <span className="max-w-[42vw] truncate rounded-md bg-muted px-2.5 py-1 text-xs font-medium text-foreground md:max-w-none">
                {profile?.fullName ?? profile?.email ?? "Profile setup required"}
              </span>
              {hasSupabaseEnv() ? (
                <form action={signOutAction}>
                  <button className="rounded-md border border-border px-3 py-1.5 text-xs font-medium text-foreground transition hover:bg-muted">
                    Sign out
                  </button>
                </form>
              ) : null}
